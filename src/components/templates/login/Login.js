...
export default function Login() {
  ...
  const onFinish = (values) => {
    ...
    AuthService.login(signId, password).then(() => {
      ...
      if (error.response && error.response.status === 400) {
        ...
        message.warning('신청한 계정은 아직 미승인 상태 입니다. 관리자에게 문의하세요.');
      } else {
  ...
  const handleGoogleLoginClick = () => {
    ...
    const googleOAuthRedirectLoginUrl = `${window.location.origin
      + adminConfig.authentication.oauthLoginResultUrl}?returnUrl=${returnUrl}`;

    window.location.href = `${siteSettings.googleWorkspaceOAuthUri}
    &state=${encodeURIComponent(googleOAuthRedirectLoginUrl)}`;
  };
  ...
  return (
    <LoginWrapper>
      ...
      <Form onFinish={onFinish}>
        ...
        <FormItem>
          <Button label="로그인" type="primary" htmlType="submit" block loading={loading} />
      ...
      <DoorayLogin
        show={showDoorayLogin}
        onLoginSuccess={handleDoorayLoginSuccess}
        onClose={() => {
          setShowDoorayLogin(false);
        }}
      />
      <MemberSignUp
        show={showMemberSignUp}
        onClose={() => {
