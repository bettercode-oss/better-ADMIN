...
import {
  Button, Dropdown, message, Modal, Table,
} from 'antd';
...
function MemberApproval() {
  ...
  const approveMember = (memberId) => {
    MemberService.approveMember(memberId).then(() => {
  ...
  const rejectMember = (memberId) => {
    MemberService.rejectMember(memberId).then(() => {
  ...
  const handleTableChanged = (changedPagination) => {
    ...
    const fullUrl = `${baseUrl}?page=${changedPagination.current}&pageSize=${changedPagination.pageSize}`;
    navigate(fullUrl);
  };
